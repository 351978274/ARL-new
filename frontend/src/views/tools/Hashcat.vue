<template>
  <div class="page-container">
    ...
    <div class="stat-cards">
      ...
      <div class="stat-card">
        <div class="stat-title">破解成功</div>
        <div class="stat-value" style="color: #f56c6c">{{ stat.cracked }}</div>
      </div>
    </div>
    ...
    <el-card shadow="never">
      <div class="table-toolbar">
        <el-button type="primary" :icon="Plus" @click="addVisible = true">新建恢复</el-button>
      ...
      <el-table :data="list" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column label="哈希类型" width="100">
          <template #default="{ row }">{{ hashTypeText(row.options && row.options.hash_type) }}</template>
        </el-table-column>
        <el-table-column label="攻击模式" width="100">
          <template #default="{ row }">{{ attackModeText(row.options && row.options.attack_mode) }}</template>
        </el-table-column>
        <el-table-column label="字典/掩码" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">{{ row.wordlist || '-' }}</template>
        </el-table-column>
        ...
        <el-table-column label="破解数" prop="result_count" width="90" />
        <el-table-column label="创建时间" prop="save_date" width="170" />
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/hashcat_result?task_id=${row._id}`)">结果</el-button>
    ...
    <HashcatDialog v-model="addVisible" @created="loadData" />
...
import HashcatDialog from './HashcatDialog.vue'
...
async function loadData() {
  ...
  try {
    ...
    const res = await hashcatApi.listTask(params)
...
// 常见哈希类型编号 → 名称（仅展示用，hashcat 有 400+ 种）
const HASH_TYPES = { 0: 'MD5', 100: 'SHA1', 1400: 'SHA256', 1800: 'sha512crypt',
                     1000: 'NTLM', 500: 'md5crypt', 3200: 'bcrypt', 13100: 'Kerberos' }
function hashTypeText(m) {
  if (m === undefined || m === null || m === '') return '自动'
  return HASH_TYPES[m] || `mode ${m}`
}
function attackModeText(a) {
  return { 0: '字典', 1: '组合', 3: '掩码', 6: '字典+掩码', 7: '掩码+字典', 8: '通用', 9: '关联' }[a] || (a !== undefined && a !== '' ? `mode ${a}` : '-')
}
function canStop(s) { return ['waiting', 'running'].includes(s) }

async function onStop(row) {
  await ElMessageBox.confirm(`确定停止恢复任务 ${row.name}？`, '提示', { type: 'warning' })
  await hashcatApi.stopTask(row._id)
...
async function onDelete() {
  ...
  await hashcatApi.deleteTask(ids)
