  ...
  <div class="page-container">
    ...
    <el-card shadow="never">
      <div class="table-toolbar">
        <el-button type="primary" :icon="Plus" @click="addVisible = true">新建破解</el-button>
      ...
      <el-table :data="list" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column label="抓包文件" min-width="180" show-overflow-tooltip>
          <template #default="{ row }">{{ captureBasename(row.capture_file) }}</template>
        </el-table-column>
        ...
        <el-table-column label="破解" width="80">
          <template #default="{ row }">
            <el-tag v-if="row.result_count > 0" type="success" size="small">成功</el-tag>
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/aircrack_result?task_id=${row._id}`)">结果</el-button>
    ...
    <AircrackDialog v-model="addVisible" @created="loadData" />
...
import AircrackDialog from './AircrackDialog.vue'
...
async function loadData() {
  ...
  try {
    ...
    const res = await aircrackApi.listTask(params)
...
function attackModeText(m) {
  return { '1': 'WEP', '2': 'WPA-PSK' }[m] || (m || '-')
}
function canStop(s) { return ['waiting', 'running'].includes(s) }
function captureBasename(p) {
  if (!p) return '-'
  return p.split(/[\\/]/).pop()
}

async function onStop(row) {
  await ElMessageBox.confirm(`确定停止破解任务 ${row.name}？`, '提示', { type: 'warning' })
  await aircrackApi.stopTask(row._id)
...
async function onDelete() {
  ...
  await aircrackApi.deleteTask(ids)
