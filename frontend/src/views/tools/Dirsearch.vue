<template>
  <div class="page-container">
    ...
    <el-card shadow="never">
      ...
      <el-table :data="list" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/dirsearch_result?task_id=${row._id}`)">结果</el-button>
    ...
    <!-- 新建扫描弹窗 -->
    <DirsearchDialog v-model="addVisible" @created="loadData" />
...
import DirsearchDialog from './DirsearchDialog.vue'
...
async function loadData() {
  ...
  try {
    ...
    const res = await dirsearchApi.listTask(params)
...
async function onStop(row) {
  ...
  await dirsearchApi.stopTask(row._id)
...
async function onDelete() {
  ...
  await dirsearchApi.deleteTask(ids)
