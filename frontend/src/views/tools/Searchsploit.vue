<template>
  <div class="page-container">
    ...
    <div class="stat-cards">
      ...
      <div class="stat-card">
        <div class="stat-title">结果条数</div>
        <div class="stat-value" style="color: #e6a23c">{{ stat.results }}</div>
      </div>
    </div>
    ...
    <el-card shadow="never">
      <div class="table-toolbar">
        <el-button type="primary" :icon="Plus" @click="addVisible = true">新建搜索</el-button>
      ...
      <el-table :data="list" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column label="关键词" min-width="240" show-overflow-tooltip>
          <template #default="{ row }">{{ (row.terms || []).join(' ') }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/searchsploit_result?task_id=${row._id}`)">结果</el-button>
    ...
    <SearchsploitDialog v-model="addVisible" @created="loadData" />
...
import SearchsploitDialog from './SearchsploitDialog.vue'
...
async function loadData() {
  ...
  try {
    ...
    const res = await searchsploitApi.listTask(params)
...
async function onStop(row) {
  await ElMessageBox.confirm(`确定停止搜索任务 ${row.name}？`, '提示', { type: 'warning' })
  await searchsploitApi.stopTask(row._id)
...
async function onDelete() {
  ...
  await searchsploitApi.deleteTask(ids)
