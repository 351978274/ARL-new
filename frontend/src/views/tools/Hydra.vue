  ...
  <div class="page-container">
    ...
    <el-card shadow="never">
      <div class="table-toolbar">
        <el-button type="primary" :icon="Plus" @click="addVisible = true">新建爆破</el-button>
      ...
      <el-table :data="list" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column label="服务" width="120">
          <template #default="{ row }">{{ (row.options && row.options.service) || '-' }}</template>
        </el-table-column>
        ...
        <el-table-column label="目标" min-width="220" show-overflow-tooltip>
          <template #default="{ row }">{{ (row.targets || []).slice(0, 3).join(', ') }}{{ (row.targets || []).length > 3 ? '...' : '' }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/hydra_result?task_id=${row._id}`)">结果</el-button>
    ...
    <!-- 新建爆破弹窗 -->
    <HydraDialog v-model="addVisible" @created="loadData" />
...
import HydraDialog from './HydraDialog.vue'
...
async function loadData() {
  ...
  try {
    ...
    const res = await hydraApi.listTask(params)
...
async function onStop(row) {
  await ElMessageBox.confirm(`确定停止爆破任务 ${row.name}？`, '提示', { type: 'warning' })
  await hydraApi.stopTask(row._id)
