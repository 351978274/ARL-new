"""URL 工具，移植自原 app/utils/url.py。"""
from __future__ import annotations

import hashlib
import os
import re
from urllib import parse
from urllib.parse import unquote, urlparse

hash_size = 199999


def urlsimilar(url: str) -> int:
    """URL 相似度哈希（移植原 urlsimilar 逻辑，保留 % 运算以兼容原值域）。"""
    url_parse = urlparse(url)
    netloc = url_parse.netloc
    path = url_parse.path[1:]
    if path == "":
        path = "/"
    ext = os.path.splitext(path)[-1]

    path = re.sub(r'\b[0-9]+\b', '0', path)
    query = unquote(url_parse.query)
    query_keys = sorted(dict(parse.parse_qsl(query)).keys())

    query_value = 0
    if len(query_keys) > 0:
        query_value = hash("-".join(query_keys)) % 98765
    if query_value:
        return query_value

    if len(path.split('/')) > 1:
        tail = path.split('/')[-1].split('.')[-1]
    elif len(path.split('/')) == 1:
        tail = path
    else:
        tail = '1'

    path_length = len(path.split('/')) - 1
    path_list = path.split('/')[:-1] + [tail]
    path_value = 0
    for i in range(path_length + 1):
        if path_length - i == 0:
            path_value += hash(path_list[path_length - i]) % 98765
        else:
            path_value += len(path_list[path_length - i]) * (10 ** (i + 1))
    path_value += hash(ext)

    netloc_hash = hashlib.md5()
    netloc_hash.update(netloc.encode('utf-8'))
    netloc_value = hash(netloc_hash.hexdigest()) % hash_size

    url_hash = hashlib.md5()
    url_hash.update(netloc.encode('utf-8'))
    url_hash.update(str(path_value + netloc_value).encode('utf-8'))
    url_value = hash(url_hash.hexdigest()) % hash_size
    return url_value


def get_hostname(url: str) -> str:
    if "://" not in url:
        url = "http://" + url
    return urlparse(url).netloc


def rm_similar_url(all_url: list[str]) -> list[str]:
    """URL 简单去似。"""
    filther_similar_url = []
    tmp_url_value = []
    for _url in all_url:
        _url = normal_url(_url)
        if not _url:
            continue
        url_value = urlsimilar(_url)
        if url_value not in tmp_url_value:
            tmp_url_value.append(url_value)
            filther_similar_url.append(_url)
    return filther_similar_url


def normal_url(url: str) -> str | None:
    """规范化 URL（去掉默认端口，补全 path）。"""
    scheme_map = {'http': 80, "https": 443}
    o = urlparse(url)
    scheme = o.scheme
    hostname = o.hostname
    path = o.path
    if scheme not in scheme_map:
        return None
    if o.path == "":
        path = "/"
    if o.port == scheme_map[o.scheme] or o.port is None:
        ret_url = f"{scheme}://{hostname}{path}"
    else:
        ret_url = f"{scheme}://{hostname}:{o.port}{path}"
    if o.query:
        ret_url = ret_url + "?" + o.query
    return ret_url


def cut_filename(url: str) -> str:
    """截取 URL 所在目录（用于 PoC 目标）。"""
    o = urlparse(url)
    dir_path = os.path.dirname(o.path)
    dir_path = dir_path.rstrip("/")
    if not o.netloc:
        return ""
    return f"{o.scheme}://{o.netloc}{dir_path}"


def same_netloc(url1: str, url2: str) -> bool:
    return get_hostname(url1) == get_hostname(url2)


async def verify_cert(url: str) -> bool:
    """异步探测证书是否可信。"""
    from . import http_req
    try:
        await http_req(url, method='head', verify=True)
        return True
    except Exception:
        return False


def url_ext(url: str) -> str:
    path = urlparse(url).path
    return os.path.splitext(path)[-1].lower()
